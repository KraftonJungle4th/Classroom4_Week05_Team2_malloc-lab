...
*/
#define GET_FREE_PREV(bp) (*(char **)(bp))
#define GET_FREE_NEXT(bp) (*(char **)(bp + WSIZE))
...
int mm_init(void)
{
    ...
    {
        printf("[mm_init] 함수 종료, ERROR: (freeListRoot = mem_sbrk(sizeof(FreeBlock))) == (void *)-1\n");
        return -1;
    }
    ...
    // 초기 힙을 확장하여 충분한 양의 메모리가 사용 가능하도록 chunksize를 단어 단위로 변환하여 힙 확장
...
void *mm_malloc(size_t size)
{
    ...
    {
        printf("[mm_malloc] 함수 종료, bp = extend_heap(extendsize / WSIZE)) == NULL");
        return NULL;
    }
...
{
    printf("[find_fit] 함수 시작, asize: %zu\n", asize);

    if (freeListRoot == NULL)
    {
        printf("[find_fit] 함수 종료, ERROR: freeListRoot == NULL\n");
        return NULL;
    }

    /* Explicit & First-fit search */
    // 가용 블럭의 next가 NULL이 아닌 동안 순회
    for (FreeBlock *freeBlock = freeListRoot->next; freeBlock->next != NULL; freeBlock = freeBlock->next)
    {
        // 가용 블럭의 사이즈가 필요 사이즈와 같거나 크면 해당 블럭 주소 반환
        if (freeBlock->size >= asize)
        {
            printf("[find_fit] 함수 종료, freeBlock 반환 (%p)", freeBlock);
            return freeBlock;
        }
    }

    printf("[find_fit] 함수 종료, asize: %zu\n", asize);
    return NULL;
}

static void place(void *bp, size_t asize)
{
    printf("[place] 함수 시작, bp: %p\n", bp);
...
static void *coalesce(void *bp)
{
    ...
    {
        printf("[coalesce] Case 1. 이전, 다음 블록이 이미 사용 중일때\n");
        insert_free_block(bp);
        return bp;
    }
    ...
    {
        printf("[coalesce] Case 2. 다음 블록이 가용일 때\n");

        // nextFreeBlock을 가용리스트에서 삭제
        nextFree = (FreeBlock *)NEXT_BLKP(bp);
    ...
    {
        printf("[coalesce] Case 3. 이전 블록이 가용일 때\n");
    ...
    {
        printf("[coalesce] Case 4. 이전과 다음 블록이 가용일 때\n");

        // 내 앞뒤 free block들의 주소를 받아와서 가용리스트에서 삭제
        prevFree = (FreeBlock *)PREV_BLKP(bp);
        nextFree = (FreeBlock *)NEXT_BLKP(bp);
...
static void verify_list_integrity(void *head)
{
    ...
    while (slow && GET_FREE_NEXT(slow) && fast && GET_FREE_NEXT(fast) && GET_FREE_NEXT(GET_FREE_NEXT(fast)))
    {
        slow = GET_FREE_NEXT(slow);
        fast = GET_FREE_NEXT(GET_FREE_NEXT(fast));
    ...
    for (void *curr = head; curr != NULL; curr = GET_FREE_NEXT(curr))
    {
        if (GET_FREE_PREV(curr) != prev)
        {
...
{
    printf("[insert_free_block] 함수 시작, newHeadBlock: %p\n", newHeadBlock);

    void *oldHeadBlock = GET_FREE_NEXT(freeListRoot);
    SET_FREE_NEXT((void *)newHeadBlock, (void *)oldHeadBlock);

    if (oldHeadBlock != NULL)
    {
        SET_FREE_PREV(oldHeadBlock, (void *)newHeadBlock);
    }

    SET_FREE_NEXT(freeListRoot, (void *)newHeadBlock);
    SET_FREE_PREV((void *)newHeadBlock, NULL);

    verify_list_integrity(freeListRoot);

    printf("[insert_free_block] 함수 종료, newHeadBlock: %p\n", newHeadBlock);
}

static void remove_free_block(FreeBlock *currBlock)
{
    printf("[remove_free_block] 함수 시작, currBlock: %p\n", currBlock);

    void *prevBlock = GET_FREE_PREV(currBlock);
    void *nextBlock = GET_FREE_NEXT(currBlock);
    ...
    printf("[remove_free_block] 함수 종료, currBlock: %p\n", currBlock);
}
